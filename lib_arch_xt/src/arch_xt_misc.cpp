...
bool ArchXT_IntCtrl::init(Device& device)
{
    ...
    add_ioreg(INT_REG_ADDR(STATUS), CPUINT_NMIEX_bm | CPUINT_LVL1EX_bm | CPUINT_LVL0EX_bm, IORegister::RO);
...
bool ArchXT_MiscRegCtrl::init(Device& device)
{
    ...
    add_ioreg(m_config.reg_revid, 0xFF, IORegister::RO);

    add_ioreg(SIGROW_REG_ADDR(DEVICEID0), 0xFF, IORegister::RO);
    add_ioreg(SIGROW_REG_ADDR(DEVICEID1), 0xFF, IORegister::RO);
    add_ioreg(SIGROW_REG_ADDR(DEVICEID2), 0xFF, IORegister::RO);

    for (int i = 0; i < 10; ++i)
        add_ioreg(SIGROW_REG_ADDR(SERNUM0) + i, 0xFF, IORegister::RO);

    add_ioreg(SIGROW_REG_ADDR(OSCCAL16M0), 0x7F, IORegister::RO);
    add_ioreg(SIGROW_REG_ADDR(OSCCAL20M1), 0x0F, IORegister::RO);
    add_ioreg(SIGROW_REG_ADDR(OSCCAL16M0), 0x7F, IORegister::RO);
    add_ioreg(SIGROW_REG_ADDR(OSCCAL20M1), 0x0F, IORegister::RO);
    add_ioreg(SIGROW_REG_ADDR(TEMPSENSE0), 0xFF, IORegister::RO);
    add_ioreg(SIGROW_REG_ADDR(TEMPSENSE1), 0xFF, IORegister::RO);
    add_ioreg(SIGROW_REG_ADDR(OSC16ERR3V), 0xFF, IORegister::RO);
    add_ioreg(SIGROW_REG_ADDR(OSC16ERR5V), 0xFF, IORegister::RO);
    add_ioreg(SIGROW_REG_ADDR(OSC20ERR3V), 0xFF, IORegister::RO);
    add_ioreg(SIGROW_REG_ADDR(OSC20ERR5V), 0xFF, IORegister::RO);
