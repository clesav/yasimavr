...
void ArchAVR_ExtInt::ioreg_write_handler(reg_addr_t addr, const ioreg_write_t& data)
{
    ...
    if (addr == m_config.rb_extint_flag) {
    ...
    else if (addr == m_config.rb_pcint_flag) {
        for (unsigned int i = 0; i < m_config.pc_ints.size(); ++i) {
...
void ArchAVR_ExtInt::pin_signal_raised(const signal_data_t& sigdata, int hooktag)
{
    ...
    if (is_pc) {
        ...
        bool enabled = test_ioreg(regbit_t(m_config.pc_ints[bank].reg_mask, bit));
        ...
        if (enabled && (pin_level ^ old_level)) {
            ...
            set_ioreg(m_config.rb_pcint_flag.bit(bank));
        ...
        if (trigger && test_ioreg(m_config.rb_extint_mask.bit(pin_num))) {
            uint8_t v = (old_level ? 2 : 0) | (pin_level ? 1 : 0);
            set_ioreg(m_config.rb_extint_flag.bit(pin_num));
